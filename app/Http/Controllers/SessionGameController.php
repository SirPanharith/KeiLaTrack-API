    ...
    public function getSessionGamesByPlayer($playerId)
{
    ...
    public function getSessionGameBySessionAndPlayer($sessionId, $playerId, Request $request)
{
    try {
        // Fetch the list of session IDs from the request, if provided
        $allSessionIds = $request->input('all_session_ids', []);
        ...
        $allSessions = SessionGame::whereIn('Session_ID', $allSessionIds)
            ->whereHas('sessionInvitations', function ($query) use ($player) {
        ...
        return response()->json([
            ...
            'All_Session_IDs' => $allSessionIds, // Include all session IDs
        ]);
    } catch (\Exception $e) {
        // Handle or log the exception
        return response()->json(['error' => $e->getMessage()], 500);
